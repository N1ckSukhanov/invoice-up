  ...
  if ($("#fullCalendar").length) {
    ...
    calendar = new FullCalendar.Calendar(document.getElementById("fullCalendar"), {
      ...
      events: function (info, successCallback, failureCallback) {
        $.ajax({
          method: "GET",
          url: `/api/reservations?from=${encodeURIComponent(info.startStr)}&to=${encodeURIComponent(info.endStr)}`,
          dataType: "JSON",
          success: function (data) {
            successCallback(data.map(e => {return {...e, start: Date.parse(e.start), end: Date.parse(e.end)}}))
          },
          error: function (err) {
            failureCallback(err);
          }
